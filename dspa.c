...
static irqreturn_t no_port_irq(int irq, void *dev_id) {
    irq_count++;
//    printk(KERN_INFO "Not ready port: counter=%d\n", irq_count);
    return IRQ_NONE;
}
...
void make_init(int driver_no) {
    switch (table_drvs[driver_no].tdrv.codedrv) {
        case DRIVER_TEST1:
            testdrv1_ini(&table_drvs[driver_no]);
            return;
        case DRIVER_TEST2:
            testdrv2_ini(&table_drvs[driver_no]);
            return;
        case DRIVER_TEST3:
            testdrv3_ini(&table_drvs[driver_no]);
            return;
    }
...
static ssize_t dev_write(struct file * file, const char * buf,
    ...
    drv_len_data[drv_count] = loff.df.len_buffer;
